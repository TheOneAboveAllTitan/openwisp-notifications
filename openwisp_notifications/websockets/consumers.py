...
from openwisp_notifications.api.serializers import ObjectNotificationSerializer
...
class NotificationConsumer(WebsocketConsumer):
    ...
    def receive(self, text_data):
        if self._is_user_authenticated():
            ...
            if json_data['type'] == 'notification':
                self._notification_handler(notification_id=json_data['notification_id'])
    ...
    def _object_notification_handler(self, object_id, app_label, model_name):
        try:
            object_notification = ObjectNotification.objects.get(
                user=self.scope['user'],
                object_id=object_id,
            ...
            serialized_data = ObjectNotificationSerializer(object_notification)
            self.send(
                json.dumps(
                    {
        ...
        except ObjectNotification.DoesNotExist:
            return
