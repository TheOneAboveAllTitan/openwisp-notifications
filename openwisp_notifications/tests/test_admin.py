...
class TestAdmin(TestOrganizationMixin, TestCase):
    ...
    def test_notification_setting_admin_has_change_permission(self):
        with self.subTest('Test for superuser'):
            self.assertTrue(self.ns_admin.has_change_permission(su_request),)

        with self.subTest('Test for non-superuser'):
            self.assertFalse(self.ns_admin.has_change_permission(op_request),)
            self.assertTrue(
                self.ns_admin.has_change_permission(op_request, obj=op_request),
            )
