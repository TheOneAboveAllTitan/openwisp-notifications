...
def notify_handler(**kwargs):
    ...
    if notification_type:
        ...
        if target_org:
            # If tagert organization is not defined, then non-superusers will not
            # have a notification setting.
...
def send_email_notification(sender, instance, created, **kwargs):
    ...
    if instance.type and (target_org or instance.recipient.is_superuser):
        ...
        # or if target_org is not present and user is not superuser
...
def notification_setting_org_user_created(instance, created, **kwargs):
    if created:
        tasks.ns_organization_user_added.delay(
...
@receiver(post_save, sender=User, dispatch_uid='superuser_notification_setting')
